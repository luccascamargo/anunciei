...
export default async function Page() {
  ...
  const activeAds = await fetchApi<
    Promise<
  ...
  const inactiveAds = await fetchApi<
    Promise<
      Prisma.AdvertsGetPayload<{
        include: { images: true; brand: true; model: true };
      }>[]
  ...
  const requestedAds = await fetchApi<
    Promise<
      Prisma.AdvertsGetPayload<{
        include: { images: true; brand: true; model: true };
      }>[]
  ...
  const [activeAdsData, inactiveAdsData, requestedAdsData] = await Promise.all([
    activeAds,
    inactiveAds,
    requestedAds,
  ]);
